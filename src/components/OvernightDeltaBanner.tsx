...
export function OvernightDeltaBanner({ delta }: Props) {
  if (delta.isFirstVisit && delta.lines.length <= 1) {
    return (
      <section className="panel px-4 py-3 border-green-400/30 bg-green-950/20">
  ...
  return (
    <section className="panel px-4 py-3 border-green-400/30 bg-gradient-to-r from-green-500/10 to-transparent">
      <p className="hud-label flex items-center gap-1.5">
        <Sparkles className="h-3.5 w-3.5" /> {delta.headline}
      </p>
